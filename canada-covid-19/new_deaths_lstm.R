...
# --- LSTM model function ---
lstm_model_fn <- function(df, m, pred_size=7, window_size=7, hidden_dim=64, num_epochs=300, batch_size=64, learning_rate=0.0075){
...
plot(second_wave_df$date, second_wave_df$numdeaths_diff_avg, col = "black", lwd = 2, type="l",
     xlab = "Time (days)", ylab="Deaths", main = "Daily new deaths - Second wave", cex=0.5)
# Add vertical lines for the prediction period
...
save(death_cases,
     ...
     df_pred_lstm,
     loss_lstm_mat,
     file = "COVID19_death_output_lstm_July11.RData")
