...
# Load data
load("COVID19_deaths_output_July11.RData")
...
# --- Import data ---
raw_df = read.csv("D:/Dalhousie University/0. Project/Data/covid19-download.csv")
...
all_waves <- ggplot(filtered_df, aes(x = date, y = numdeaths_diff_avg)) +
...
all_waves # show the plot
ggsave("all_waves_new_deaths_plot.pdf", plot = all_waves, width = 6, height = 3, units = "in")

# --- Visualize the second wave of daily deaths ---
...
for (i in 1:nrow(df_true)){
  past_vec[i] <- death_case[df_true[i,1]]
}

# Data frame for vertical lines
vline_data <- data.frame(
  xintercept = c(m, 135, 126, 151, 167), # c(m, 135, 144, 155, 126, 139, 151, 167),
  type = c("First Training End", "Trough", 
           "Peak", "Peak", "Peak")
)
vline_data$type <- factor(vline_data$type, levels = c("First Training End", "Peak", "Trough"))

p_2ndwave <- ggplot(second_wave_df, aes(x = seq_along(numdeaths_diff_avg), y = numdeaths_diff_avg)) +
  geom_line(color = "black", size = 1) +
...
# --- Random Forest ---
# Load outputs
load("D:/Dalhousie University/0. Project/Output/COVID19 v3/Deaths/COVID19_deaths_output_rforest_July11.RData")

df_nrow = nrow(df_true)
df_ncol = ncol(df_true)
start_index = 100
end_index = length(death_case)
...
# --- LSTM ---
# Load output
load("D:/Dalhousie University/0. Project/Output/COVID19 v3/Deaths/COVID19_death_output_lstm_July11.RData")
...
ggsave("new_deaths_pred_plot.pdf", plot = pred_plot, width = 12, height = 10, units = "in")
...
ggsave("new_deaths_MDA_plot.pdf", plot = MDA_plot, width = 9, height = 5, units = "in")
...
relative_error_plot
ggsave("new_deaths_relative_error_plot.pdf", plot = relative_error_plot, width = 5, height = 5, units = "in")
