...
# Save results for RF
## ESS plots
pdf("train108_RF_ess.pdf", width=7, height=5)
par(mfrow=c(1, 1))
for (i in 1:n_simulation){
...
## Residuals
pdf("train108_RF_residuals.pdf", width=10, height=4)
par(mfrow=c(1, 2))
...
pdf("train108_RF_coefs_samples.pdf", width=15, height=20)
par(mfrow=c(6, 3))
...
## Sigma2 of last iter
pdf("train108_RF_sigma2_samples.pdf", width=15, height=4)
par(mfrow=c(1, 3))
...
## ESS plots
pdf("train108_rfBLT_ess.pdf", width=7, height=5)
par(mfrow=c(1, 1))
for (i in 1:n_simulation){
...
## Residuals
pdf("train108_rfBLT_residuals.pdf", width=10, height=4)
par(mfrow=c(1, 2))
...
## Coefficients of last iter
pdf("train108_rfBLT_coefs_samples.pdf", width=15, height=20)
par(mfrow=c(6, 3))
...
## Sigma2 of last iter
pdf("train108_rfBLT_sigma2_samples.pdf", width=15, height=4)
par(mfrow=c(1, 3))
...
# Bayes Lasso
m108_pred_bayes_lasso = bayes_lasso_y_pred
m108_lowerCI_bayes_lasso = bayes_lasso_lowerCI
m108_upperCI_bayes_lasso = bayes_lasso_upperCI

# Bayes Lasso Takens with Smoothing Derivatives
m108_pred_BLT_normal_error <- bayes_lasso_taken_normal_y_pred
m108_lowerCI_normal_error <- bayes_lasso_taken_normal_lowerCI
m108_upperCI_normal_error <- bayes_lasso_taken_normal_upperCI

# --- ARIMA ---
m108_arima_pred_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_arima_lowerCI_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_arima_upperCI_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_arima_avg_noise_aic <- numeric(n_simulation)
m108_arima_p <- numeric(n_simulation)
m108_arima_d <- numeric(n_simulation)
m108_arima_q <- numeric(n_simulation)

# I noise m = 108
for (i in 1:n_simulation){
  arima <- arima_model(I_noise_avg_mat[i,], m=fixed_point, pred_size=pred_size)
  m108_arima_pred_avg_noise[i,] <- arima$pred
  m108_arima_lowerCI_avg_noise[i,] <- arima$lowerCI
  m108_arima_upperCI_avg_noise[i,] <- arima$upperCI
  m108_arima_avg_noise_aic[i] <- arima$aic
  m108_arima_p[i] <- arima$model$arma[1]
  m108_arima_q[i] <- arima$model$arma[2]
  m108_arima_d[i] <- arima$model$arma[6]
}

# --- Holt Exponential Smoothing ---
m108_holt_pred_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_holt_lowerCI_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_holt_upperCI_avg_noise <- matrix(rep(NA, (n_simulation*pred_size)), ncol=pred_size, nrow=n_simulation, byrow=TRUE)
m108_holt_avg_noise_aic <- numeric(n_simulation)
...
for (i in 1:n_simulation){
  ...
  m108_holt_pred_avg_noise[i,] <- as.vector(fit_holt$mean)
  m108_holt_lowerCI_avg_noise[i,] <- as.vector(fit_holt$lower[,2])
