...
# --- Load the S&P500 index from quantmod ---
symbol <- "^GSPC"
start_date <- "2022-01-15"
end_date <- "2025-05-31"
fixed_point <- 753 # end of the first training period

# Get stock data for the specified period
getSymbols(symbol, from = start_date, to = end_date)
df = GSPC$GSPC.Close
prices <- as.vector(df$GSPC.Close)
n <- length(df$GSPC.Close)

sp500_plot <- ggplot(df, aes(x = index(df), y = GSPC.Close)) +
  geom_line(color = "black", size = 1) +
  geom_vline(xintercept = index(df)[fixed_point], color = "blue", linetype = "dashed") +
  labs(# title = "Daily closing values of S&P500 from January 15, 2022 to May 31, 2025",
       x = "Date", y = "Closing values") +
  theme_minimal() +
  theme(plot.title = element_text(size = 11),
        axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9))

sp500_plot # show plot
ggsave("sp500_plot.pdf", plot = sp500_plot, width = 9, height = 5, units = "in")

# --- Load outputs from models --- 
load("D:/Dalhousie University/0. Project/Output/SP500 v3/SP500_without_LSTM_output_July11.RData")

# --- Visualize different subperiods ---
past_vec <- numeric(nrow(df_true))
for (i in 1:nrow(df_true)){
  past_vec[i] <- df$GSPC.Close[df_true[i,1]]
}
...
new_df <- data.frame(
  sp500 = as.vector(df$GSPC.Close[start:end]),
  day = start:end)

p_pred_period <- ggplot(new_df, aes(x = day, y = sp500)) +
  geom_line(color = "black", size = 1) +
  geom_vline(data = vline_data,
             aes(xintercept = xintercept, color = type, linetype = type),
             size = 0.8, show.legend = TRUE) +
  labs(title = "Prediction period of S&P500 index",
       x = "Day", y = "S&P500",
...
# --- Random Forest ---
# Load outputs
load("D:/Dalhousie University/0. Project/Output/SP500 v3/SP500_rforest_output_July11.RData")
...
# --- LSTM ---
# Load outputs
load("D:/Dalhousie University/0. Project/Output/SP500 v3/SP500_LSTM_output_July11.RData")
...
final_plot <- p_pred_period/(p1|p2|p3)/(p4|p5|p6)
...
ggsave("sp500_pred_plot.pdf", plot = pred_plot, width = 12, height = 10, units = "in")
...
MDA_plot # show plot
ggsave("sp500_MDA_plot.pdf", plot = MDA_plot, width = 9, height = 5, units = "in")
...
relative_error_plot # show plot
ggsave("sp500_relative_error_plot.pdf", plot = relative_error_plot, width = 5, height = 5, units = "in")
